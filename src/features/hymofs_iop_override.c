					  ...
					  unsigned int query_flags)
{
	...
	atomic64_inc(&hymo_hook_stats.iop_getattr_entries);
	rcu_read_lock();
	...
	if (ret == 0 && inode && inode->i_mapping &&
	    test_bit(AS_FLAGS_HYMO_SPOOF_KSTAT, &inode->i_mapping->flags)) {
		hymo_apply_kstat_spoof(inode, stat);
		atomic64_inc(&hymo_hook_stats.iop_getattr_spoofs);
	}
	return ret;
}
...
int hymofs_iop_mark_spoof(struct inode *inode)
{
	...
	return hymofs_iop_install(inode);
}
...
int hymofs_iop_override_init(void)
{
	...
	pr_info("HymoFS: iop_override initialized (cleanup via sop_override)\n");
	return 0;
}
